...
class _ManagerMaintenanceScreenState extends State<ManagerMaintenanceScreen>
      ...
      children: [
        ...
        Expanded(
          child: StreamBuilder<List<MaintenanceModel>>(
            stream: _maintenanceService.getMaintenanceByProperty(user.assignedPropertyId!),
            builder: (context, snapshot) {
              ...
              if (allRequests.isEmpty) {
                return const EmptyState(
                  ...
                  subtitle: 'Tenant maintenance requests will appear here',
                );
              }
              ...
              Row(
                ...
                children: [
                  if (request.status == MaintenanceStatus.pending)
                    OutlinedButton.icon(
                      onPressed: () => _showResponseDialog(request, MaintenanceStatus.inProgress),
                      icon: const Icon(Icons.play_arrow, size: 18),
                      label: const Text('Start Work'),
                    ),
                  const SizedBox(width: 8),
                  ElevatedButton.icon(
                    onPressed: () => _showResponseDialog(request, MaintenanceStatus.fixed),
  ...
  void _showResponseDialog(MaintenanceModel request, MaintenanceStatus newStatus) {
    final noteController = TextEditingController();
    final statusText = newStatus == MaintenanceStatus.inProgress ? 'In Progress' : 'Fixed';
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Add a response note for the tenant (optional)'),
            const SizedBox(height: 16),
            CustomTextField(
              controller: noteController,
              label: 'Response Note',
              hint: 'e.g., Plumber scheduled for tomorrow',
              maxLines: 3,
            ),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              final success = await _maintenanceService.updateMaintenanceStatus(
                request.id,
                newStatus,
                responseNote: noteController.text.trim().isEmpty
                    ? null
